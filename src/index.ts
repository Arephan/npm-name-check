...
function parseArgs(): Options {
  ...
  const options: Options = { names: [], similar: false, help: false };

  for (const arg of args) {
    if (arg === "-h" || arg === "--help") options.help = true;
    else if (arg === "-s" || arg === "--similar") options.similar = true;
    else if (!arg.startsWith("-")) options.names.push(arg.toLowerCase());
  }
...
function checkName(name: string): Promise<{ name: string; available: boolean; error?: string }> {
...
function generateSimilar(name: string): string[] {
...
function isValidNpmName(name: string): { valid: boolean; reason?: string } {
  if (name.length === 0) return { valid: false, reason: "Name cannot be empty" };
  if (name.length > 214) return { valid: false, reason: "Name too long (max 214)" };
  if (name.startsWith(".") || name.startsWith("_")) return { valid: false, reason: "Cannot start with . or _" };
  if (name !== name.toLowerCase()) return { valid: false, reason: "Must be lowercase" };
  if (/[~'!()*]/.test(name)) return { valid: false, reason: "Contains invalid characters" };
  if (encodeURIComponent(name) !== name) return { valid: false, reason: "Contains invalid characters" };
